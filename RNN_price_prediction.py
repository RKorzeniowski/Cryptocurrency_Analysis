...
train_ab = traning_presplit.iloc[:2000]
validate_ab = traning_presplit.iloc[2000:2116]
test_ab = traning_presplit.iloc[2116:2232]
...
dataset_test = test_ab
real_stock_price = test_ab
...
dataset_total = pd.concat((train_ab['avg_btc_price_usd'], test_ab['avg_btc_price_usd']), axis = 0)

#inputs = dataset_total[len(dataset_total) - len(dataset_test) - time_predicting:].values
#inputs = inputs.reshape(-1,1)
#inputs = scaler.transform(inputs)

inputs = dataset_total[len(dataset_total) - len(test_ab) - time_steps:].values
inputs = inputs.reshape(-1,1)
inputs = scaler.transform(inputs)
X_test = []
